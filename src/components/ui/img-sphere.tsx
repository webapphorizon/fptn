...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    setIsDragging(true);
    setVelocity({ x: 0, y: 0 });
    lastMousePos.current = { x: touch?.clientX ?? 0, y: touch?.clientY ?? 0 };
  }, []);
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    const touch = e.touches[0];
    const deltaX = touch?.clientX ?? 0 - lastMousePos.current.x;
    const deltaY = touch?.clientY ?? 0 - lastMousePos.current.y;
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative h-full w-full overflow-hidden rounded-full dark:border-none border-2  shadow-lg">
          <Image
            src={image.src}
            alt={image.alt}
            width={100}
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-transparent rounded-lg animate-pulse flex items-center justify-center"
